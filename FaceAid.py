...
def update_camera():
    ...
    if ret:
        ...
        if results.multi_face_landmarks:
            ...
            if is_paused:
                ...
                if ear < BLINK_THRESHOLD: 
                    ...
                    if int(elapsed) > last_beep and int(elapsed) < EYES_CLOSED_TIME:
                        play_sound(500 + (int(elapsed) * 500), 200)
                        last_beep = int(elapsed)
        ...
        cv2.imshow('FaceAid Camera', frame)
        cv2.waitKey(1)
...
scale_dead.set(int(DEADZONE_SIZE*100))
scale_dead.pack(fill="x", padx=10)

tk.Label(settings_frame, text="Smile Sensitivity", bg="#34495e", fg="#bdc3c7").pack(anchor="w", padx=10, pady=(10,0))
...
chk_mute = tk.Checkbutton(settings_frame, text="Mute Sounds", variable=mute_var, 
...
tk.Button(root, text="QUIT", command=quit_program, bg="#c0392b", fg="white", font=("Segoe UI", 12, "bold"), height=2).pack(fill="x", side="bottom", padx=20, pady=20)
